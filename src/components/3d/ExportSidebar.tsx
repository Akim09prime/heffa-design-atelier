...
export const ExportSidebar: React.FC<ExportSidebarProps> = ({ 
  ...
  const handlePDFExport = () => {
    ...
    for(let i = 1; i <= pageCount; i++) {
      doc.setPage(i);
      doc.text('HeffaDesign - Custom Furniture Solutions', 105, doc.internal.pageSize.height - 10, { align: 'center' });
      doc.text(`Page ${i} of ${pageCount}`, 195, doc.internal.pageSize.height - 10);
    }
    
    // Save the PDF
    doc.save(`${projectName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`);
  };
  ...
  const handleExcelExport = () => {
    ...
    // Save file
    saveAs(blob, `${projectName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
